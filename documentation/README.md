...
- **Wishes List** filters Supabase queries by `user_id` to show only the signed-in user's wishes and renders a mobile-first list with image/placeholder, chevron and tappable rows. Each row shows a title, contextual prompts for missing description, link and price treatments. A count badge appears in the header, and if the user has a slug, compact **Voir** and **Partager** actions sit in the top right with a trust note below. A blue reminder badge appears when no wish is public. The first visit shows a dismissable clipboard tip and up to two ghost rows encourage adding more wishes. Skeleton loading, friendly empty/error states and a single centered “+” FAB round out the experience. See `wishes-list-page.md` for details.
