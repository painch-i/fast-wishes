...
- **Wish Sheet** is a unified bottom sheet/drawer used to add or edit a wish. It captures title, price with currency and “≈” toggle, a single merchant link field with paste button and inline domain/title preview, personal comment, priority chips and a freeform tag. When a wish is saved the user's `user_id` is attached and the sheet closes with a single success toast. See `wish-sheet.md` for details.
